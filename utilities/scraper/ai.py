...
1. MATH CONVERSION: Ensure all mathematical variables and expressions are cleanly formatted using standard LaTeX (e.g., $n$, $n \times m$, $998\,244\,353$) so they render properly on the frontend. Whenever you reference a specific variable name (like m, n, k, x, y, etc.) in the description text, wrap it in a <var> HTML tag so the frontend can style it distinctly, and include the LaTeX form after it in parentheses. Example: write '<var>m</var> ($m$)' instead of just 'm'. This lets the frontend visually distinguish variable references from ordinary words.
2. DESCRIPTION: Rewrite the problem description so it reads like a clean, standalone coding challenge (like LeetCode). Strip out all competitive programming I/O boilerplate (e.g., ignore mentions of "the first line contains t test cases", "standard input", or raw stream reading). Focus entirely on explaining the core logical task using the input variables provided to the function. IMPORTANT: Do NOT oversimplify or remove important logical details. Keep the full reasoning, edge cases, tie-breaking rules, and any conditional logic from the original problem. If the original has multiple scenarios (e.g., tie-breaking rules, special conditions), describe each one clearly. The description should be complete enough that someone understands the entire problem without guessing. Preserve all numerical bounds and constraints mentioned in the original.
